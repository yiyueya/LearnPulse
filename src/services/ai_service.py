...
from src.utils.quota_tracker import MiniMaxQuotaTracker, MiniMaxVLMError
...
class AIService:
    ...
    def __init__(self):
        ...
        self.batch_cache = {}
        # 额度追踪器
        self.quota_tracker = MiniMaxQuotaTracker()
...
如果返回的内容不是纯JSON格式，将被视为失败。"""
        ...
        while retries < max_retries:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 529:
                        self.quota_tracker.mark_exhausted()
                        raise MiniMaxVLMError(
                            f"MiniMax API 返回 529 (Server Overloaded)，额度已标记为用尽"
                        )
                    retries += 1
    ...
    def understand_images_batch(self, image_paths, prompt="描述这些图片的内容", progress_callback=None):
        ...
        # 检查额度
        if not self.quota_tracker.can_process(len(image_paths)):
            status = self.quota_tracker.get_status()
            raise MiniMaxVLMError(
                f"图片额度不足以处理 {len(image_paths)} 张图片"
                f"（已用 {status['used']}/{status['limit']}），"
                f"将在 {status['next_reset_at']} 额度重置后继续处理"
            )
    ...
    def _create_size_based_batches(self, image_paths):
        ...
        current_batch = []
        current_size = 0
        max_size_bytes = MAX_MERGED_SIZE_MB * 1024 * 1024
    ...
    def evaluate_answer(self, question, answer):
        ...
        try:
            ...
            payload = {
                "model": "MiniMax-M2.7",
                "messages": [
                    {"role": "system", "content": "你是一个教育专家，擅长评估学生答案的正确性。"},
                    {"role": "user", "content": f"评估以下答案是否正确，并提供解析:\n题目: {question}\n答案: {answer}"}
                ]
            }
